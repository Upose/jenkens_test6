      ...
      <div class="form-content">
        ...
        <el-form-item label="授权访问名单" prop="acessAll" v-if="postForm.isLoginAcess == 1">
          <el-radio-group v-model="postForm.acessAll">
            ...
            <el-button size="medium" class="m-l" v-if="postForm.acessAll == false" type="primary"
              @click="selectUserShow()">选择用户</el-button>
          </el-radio-group>
        </el-form-item>
        ...
        <el-form-item label="设置审核流程" class="m-t" v-if="postForm.isOpenAudit == 1">
          <div class="audit-warp">
            <span class="next-txt active"><i class="m-icon iconfont el-icon-vip-zhuanxie"></i><span><i
                  class="iconfont el-icon-vip-gou2"></i> 撰稿</span></span>
            <span class="next-icon"><i class="next-bj"></i></span>
            <span class="next-txt" @click="checkClick('0')"
              :class="postForm.auditFlow.indexOf('0') > -1 ? 'active' : ''"><i
                class="m-icon iconfont el-icon-vip-zhuanxie"></i><span><i class="iconfont el-icon-vip-gou2"></i>
                提交</span></span>
            <span class="next-icon"><i class="next-bj"></i></span>
            <span class="next-txt" @click="checkClick('1')"
              :class="postForm.auditFlow.indexOf('1') > -1 ? 'active' : ''"><i
                class="m-icon iconfont el-icon-vip-zhuanxie"></i><span><i class="iconfont el-icon-vip-gou2"></i>
                初审</span></span>
            <span class="next-icon"><i class="next-bj"></i></span>
            <span class="next-txt" @click="checkClick('2')"
              :class="postForm.auditFlow.indexOf('2') > -1 ? 'active' : ''"><i
                class="m-icon iconfont el-icon-vip-zhuanxie"></i><span><i class="iconfont el-icon-vip-gou2"></i>
                初校</span></span>
            <span class="next-icon"><i class="next-bj"></i></span>
            <span class="next-txt" @click="checkClick('3')"
              :class="postForm.auditFlow.indexOf('3') > -1 ? 'active' : ''"><i
                class="m-icon iconfont el-icon-vip-zhuanxie"></i><span><i class="iconfont el-icon-vip-gou2"></i>
                复审</span></span>
            <span class="next-icon"><i class="next-bj"></i></span>
            <span class="next-txt" @click="checkClick('4')"
              :class="postForm.auditFlow.indexOf('4') > -1 ? 'active' : ''"><i
                class="m-icon iconfont el-icon-vip-zhuanxie"></i><span><i class="iconfont el-icon-vip-gou2"></i>
                二校</span></span>
            <span class="next-icon"><i class="next-bj"></i></span>
            <span class="next-txt" @click="checkClick('5')"
              :class="postForm.auditFlow.indexOf('5') > -1 ? 'active' : ''"><i
                class="m-icon iconfont el-icon-vip-zhuanxie"></i><span><i class="iconfont el-icon-vip-gou2"></i>
                终审</span></span>
            <span class="next-icon"><i class="next-bj"></i></span>
            <span class="next-txt" @click="checkClick('6')"
              :class="postForm.auditFlow.indexOf('6') > -1 ? 'active' : ''"><i
                class="m-icon iconfont el-icon-vip-zhuanxie"></i><span><i class="iconfont el-icon-vip-gou2"></i>
                终校</span></span>
            <span class="next-icon"><i class="next-bj"></i></span>
            <span class="next-txt active"><i class="m-icon iconfont el-icon-vip-zhuanxie"></i><span><i
                  class="iconfont el-icon-vip-gou2"></i> 发布</span></span>
          </div>
          <!--撰稿和发布是必须的-->
        </el-form-item>
        <el-form-item class="m-top">
          <el-button icon="iconfont el-icon-vip-quxiao" size="medium" @click="backHistory()">取消</el-button>
          <el-button icon="iconfont el-icon-vip-shangyibu" size="medium" type="primary" @click="preStep()">上一步
          </el-button>
          <el-button icon="iconfont el-icon-vip-baocun1" size="medium" type="primary" :loading="btnLoading"
            @click="submitForm('postForm')" v-button-debounce>确认</el-button>
        </el-form-item>
      </div>
    </el-form>
    <selectUser :dataList="select_user_data" @selectUserHide="selectUserHide" @getCheckUser="getCheckUser"
      v-if="select_user_show"></selectUser>
  </div>
</template>
