    ...
    this.http
      ...
      .then((res) => {
        if (res.data) {
          ...
          this.isshow_link =
            JSON.stringify(this.columnDeatils.extensionKV).indexOf("JumpLink") >
            -1;
          this.isshow_ParentCatalogue =
            JSON.stringify(this.columnDeatils.extensionKV).indexOf(
              "ParentCatalogue"
            ) > -1;
          var list = {
            title: [
          ...
          if (!this.$route.query.id) {
            ...
            this.postForm["nextAuditStatus"] =
              res.data.contentDefaultAuditStatusKV || [];
          }
        }
      })
      .catch((err) => {});
      ...
      this.http
        ...
        .then((res) => {
          ...
          if (this.postForm.parentCatalogue) {
            this.parentCatalogue_length =
              this.postForm.parentCatalogue.length || 0;
          }
            ...
            if (c_list.length > 1) {
              for (var i = 0; i < c_list.length; i++) {
                if (_this.coumn_list[i]) {
                  ...
                  if (c_list[i] != "")
                    _this.coumn_list.push({ value: c_list[i] });
                }
              }
            }
          }
  ...
  methods: {
    ...
    inputBlur() {
      if (this.postForm.parentCatalogue.length > 20) {
        this.postForm.parentCatalogue = this.postForm.parentCatalogue.slice(
          0,
          20
        );
      }
