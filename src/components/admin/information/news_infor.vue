...
export default {
  ...
  methods: {
    ...
    isAuth(name) {
      ...
      let curAuth =
        (authList && authList.find((item) => item.permission == "新闻管理")) ||
        {};
      let id = this.$route.query.id || "";
      let subMenu =
        (curAuth &&
          curAuth.permissionNodes &&
          curAuth.permissionNodes.find(
            (item) => item.component == "/admin_programInfo?id=" + id
          )) ||
        {};
      let curSonAuth =
        subMenu &&
        Array.isArray(subMenu.listPermission) &&
        subMenu.listPermission.includes(name);
      ...
      this.http
        ...
        .then((res) => {
          ...
          var list = {
            title: [
              {
                name: this.columnDeatils.columnName,
                path: {
    ...
    previewPage(id) {
      ...
      this.http
        .getPlain_url(
          ...
          "/" + this.$route.query.id + "?contentid=" + id
        )
        .then((res) => {
          news_details = res.data.content || {};
          window.localStorage.setItem(
            "news-page-preview",
            JSON.stringify(news_details)
          );
          setTimeout(() => {
    ...
    //排序提交
    byIndex(type, sourceID, sortIndex, TargetCataID, isUp) {
      //ghost 拖动 alertNum 输入序号
      var _this = this;
      if (type == "ghost") {
        //拖动排序
        this.http
          .postJsonParameter_url(
            "news-sort-content-by-target",
            { sourceID: sourceID, targetCataID: TargetCataID, isUp: isUp },
            "/" + this.postForm["columnID"]
          )
          .then((res) => {
        ...
        this.http
          .postJsonParameter_url(
            "news-sort-content-by-index",
            { sourceID: sourceID, sortIndex: sortIndex },
            "/" + this.postForm["columnID"]
          )
          .then((res) => {
            _this.$message({ type: "success", message: "排序成功!" });
    ...
    handleAudit(row) {
      ...
      if (row.nextAuditBottonName == "提交") {
        ...
        content = "是否发布该新闻?";
        if (this.columnDeatils.isOpenAudit == 1) {
          //无审核流程按钮：取消，发布  1表示有审核
          l_btn = "退回";
          r_btn = "发布";
        } else {
          l_btn = "取消";
            ...
            if (r_btn == "退回") {
              ...
              this.http
                .postJsonParameter_url(
                  ...
                  {
                    contentID: row.id,
                    auditStatus: row.nextAuditStatus[0].key,
                  },
                  "/" + this.postForm["columnID"]
                )
                .then((res) => {
    ...
    newsAdd() {
      this.$router.push({
        path: "/admin_addNews",
        query: { c_id: this.$route.query.id },
      });
    },
    //编辑新闻
    handleEdit(row) {
      this.$router.push({
        ...
        query: {
          ...
          of: this.columnDeatils.hasPermission ? 1 : "",
        },
      });
    },
    //新闻日志
    handleLog(row) {
      this.$router.push({
        path: "/admin_newsLog",
        query: { id: row.id, c_id: this.$route.query.id, title: row.title },
      });
    },
    /**表格选择框**/
